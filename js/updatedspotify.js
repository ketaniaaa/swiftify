...
sHeight = vHeight,
thickness = 35, 
colourArray = data.map(k => k.color),
el = d3.select("#topTrackChart"),
radius = Math.min(sWidth, sHeight)/2, //simple mafs 
color = d3.scaleOrdinal().range(colourArray);

const max = d3.max(data, (maxD) =>
maxD.value);


const svg = el.append('svg')
.attr('viewBox', `0 0 ${vWidth + thickness} ${vHeight + thickness}`)
.attr('id', 'toptrackaud')
.attr('width', vWidth)
.attr('height', sHeight);
...
.attr('transform', `translate( ${ (sWidth / 2) + (thickness / 2) }, ${ (sHeight / 2) + (thickness / 2)})`);
